...
5. **Adjudicate and publish.** Read every non-green claim; rewrite each note so it says what is wrong and ends with `Suggest: '...'` where wording should change (merged verifier and adversary text is raw material, not the final note; the page pre-fills the reviewer's **New wording** box from that phrase). `build_review.py <work_dir>`; open `review.html`. For a web page input the review shows the extracted text, not the site's own layout; that is deliberate. Write `FINDINGS.md`: totals, then every non-green claim in severity order with its suggested wording, then which sources were unreachable or thin, which mode ran, and which model ran the adversary (light mode: say the adversarial pass was skipped and what it would have caught). Done when the page shows zero orphans and the findings cover every non-green claim.
