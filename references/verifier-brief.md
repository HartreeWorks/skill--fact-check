...
1. A quote you report must be found by string search in a file under `sources/`, or copied verbatim from a page you fetched in this session. Never quote from memory. Sources are saved as Markdown where possible (web pages, Google Docs) and plain text for PDFs; copy sentences with or without the Markdown markup, but never change the words. Copy whole sentences exactly as they appear; never stitch fragments with "..." and never tidy punctuation, because a script re-finds every quote by exact match and rejects any it cannot find. Search with fixed strings (`grep -F` or Python `str.find`), not regular expressions. Record `verified_against` as the file name, or the URL with "(fetched today)".
